<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
    <div class="bg-white p-6 rounded shadow-lg w-96">
      <h4 class="font-bold mb-4 text-lg text-red-600">Delete Board</h4>
      <p class="mb-4">Are you sure you want to delete the board <span class="font-bold">"{{ board?.name }}"</span>?</p>
      <p class="mb-2 text-sm text-gray-500">Type the board name to confirm:</p>
      <input
        v-model="input"
...
const tryDelete = () => {
  if (input.value === props.board?.name) emit('delete')
}
</script>